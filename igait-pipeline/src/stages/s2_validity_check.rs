...
async fn validate (
    ...
    let is_skipping = output.skip_to_stage.map_or(false, |skip| skip >= 2);
    ...
    if is_skipping {
        ...
        if !tokio::fs::try_exists(&converted_front).await
            ...
            anyhow::bail!("Skipping to Stage 2 but required file from Stage 1 doesn't exist: {}", converted_front.display());
        }
        if !tokio::fs::try_exists(&converted_side).await
            .context("Failed to check if converted side video exists")? {
            anyhow::bail!("Skipping to Stage 2 but required file from Stage 1 doesn't exist: {}", converted_side.display());
        }
        logs.push_str("Skipping to Stage 2: Verified required files from Stage 1 exist\n");
    } else {
...
pub async fn execute(
    ...
    let status = match validate(output, &mut logs).await {
        Ok(status) => Ok(status),
        Err(e) => {
