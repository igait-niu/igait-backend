...
impl StageProcessor for ReframingProcessor {
    ...
    async fn process(&self, request: StageJobRequest) -> StageJobResult {
        ...
        println!("Processing job {}: Reframing", request.job_id);
        ...
        match self.do_reframing(&request, &mut logs).await {
            ...
            Err(e) => {
                let duration = start_time.elapsed();
                eprintln!("Reframing failed for job {}: {}", request.job_id, e);
...
async fn main() -> Result<()> {
    ...
    println!("Starting Stage 3 Reframing service on port {}", port);
