...
impl StageProcessor for MediaConversionProcessor {
    ...
    async fn process(&self, request: StageJobRequest) -> StageJobResult {
        ...
        info!("Processing job {}: Media Conversion", request.job_id);
        ...
        match self.do_conversion(&request, &mut logs).await {
            ...
            Err(e) => {
                let duration = start_time.elapsed();
                error!("Conversion failed for job {}: {}", request.job_id, e);
...
async fn main() -> Result<()> {
    ...
    info!("Starting Stage 1 Media Conversion service on port {}", port);
