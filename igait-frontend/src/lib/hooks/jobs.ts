	...
	const unsubscribe = onValue(
		...
		(snapshot) => {
			...
			const jobs: Job[] = Array.isArray(data) 
				? data.filter((job): job is Job => job !== null && job.email !== 'placeholder@placeholder.com')
				: Object.values(data).filter((job): job is Job => job !== null && (job as Job).email !== 'placeholder@placeholder.com');
